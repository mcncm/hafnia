//! This crate contains procedural macros used internally by the Hafnia compiler.
...
/// Builds a Hafnia error struct implementing Diagnostic. This is supposed to
...
pub fn diagnostic(input: TokenStream) -> TokenStream {
    ...
    impl_hafnia_error_macro(input)
}

fn impl_hafnia_error_macro(ast: DeriveInput) -> TokenStream {
    let name = &ast.ident;
    ...
    let spans_flat = spans.iter().map(|(field, msg)| {
        ...
        match &field.ty {
            syn::Type::Path(p) if p.path.is_ident("Span") => {
                ...
                quote! {
                    spans.push(crate::hafnia_errors::SpanReport {
                        span: self.#ident,
                        msg: #msg,
                    });
                }
            ...
            _ => {
                quote! {
                    spans.extend(self.#ident.iter().map(|span| {
                        crate::hafnia_errors::SpanReport {
                            span: *span,
                            msg: #msg,
                        }
                    }));
    ...
    let expanded = quote! {
        ...
        impl crate::hafnia_errors::Diagnostic for #name {
            ...
            fn spans<'a>(&'a self) -> Vec<crate::hafnia_errors::SpanReport<'a>> {
