    ...
    ($($name:ident $($x:ident)? $([$phase:ident])? { $($src:tt)* })*) => {
        $(
            ...
            pub fn $name() {
                ...
                let mut conf = cavy_core::session::Config::default();
                $(conf.phase_config.last_phase = cavy_core::session::Phase::$phase;)?
                ...
                if let Err(errs) = circ {
                    eprintln!("{}", cavy_core::util::FmtWith::fmt_with(&errs, &ctx));
                    panic!();
                }
            }
        )*
