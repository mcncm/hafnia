...
pub struct HafniaError;

impl std::fmt::Display for HafniaError {
...
/// `HafniaError` carries no data of its own. It's also convenient to use a name
/// other than `Result` because we don't have to write `std::result::Result`
/// everywhwere we want to specify an error type.
pub type Maybe<T> = std::result::Result<T, HafniaError>;
...
impl ErrorBuf {
    ...
    pub fn push<T: 'static + Diagnostic>(&mut self, err: T) -> HafniaError {
